...
var gl, angle=0, hing=-44.0, direction=1;
var matStack = [];
...
function render() {
   ...
   matStack.push(mv);   //mv normal
   angle = angle + 1;
   mv = mult(mv, rotateY(angle));

    matStack.push(mv);  //mv with rotateY
    mv = mult(mv, scale(1.0, 2.0, 1.0));

    gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mv)));
    gl.drawArrays(shapes.solidCube.type, shapes.solidCube.start, shapes.solidCube.size);
    mv = matStack.pop();    //undoing (back to rotateY version)

    matStack.push(mv);  //pushing rotateY version
    mv = mult(mv, translate(0.5, 1.0, 0));

    if(-45.0 < hing && hing < 45.0 && direction == 1.0){
        hing += 0.5;
    }
    else if(-45 < hing && hing < 45 && direction == 0.0)
        hing -= 0.5;
    else if(hing == 45){
        direction = 0;
        hing -= 0.5;
    ...
    mv = mult(mv, rotate(hing,vec3(0,0,1)));
    mv = mult(mv, translate(0.5, 0.5, 0));

    gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mv)));
    gl.drawArrays(shapes.solidCube.type, shapes.solidCube.start, shapes.solidCube.size);
    mv = matStack.pop();    //back to rotateY
    mv = matStack.pop();    //back to normal
