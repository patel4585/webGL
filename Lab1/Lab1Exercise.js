...
function PreRenderScene() {
 ...
 function RenderScene() {
    ...
    mv = mult(mv, translate(0.0, 0.0, -0.7));
...
function setupEventListeners()
{
  ...
  m.addEventListener("change", function(event) {
    ...
    //Event listener to change the rotation direction when 'C' or 'c' is pressed
    let temp = document.addEventListener("keydown", function(event){
      if(event.key === "C" || event.key === "c")
         rotChange *= -1;
    });

    //Event listener to change rotation speed based on slider value
    let temp2 = document.getElementById("range_1");

    temp2.addEventListener("change", function(event){
      if(rotChange < 0)
         rotChange = (-1 * event.target.value);
      else
         rotChange = (1 * event.target.value);  //Here, multipling by 1 to type coerce from string to float
