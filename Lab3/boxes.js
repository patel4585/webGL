...
var shapes = {
   ...
   wireCube_2: {points:[], colors:[], start:0, size:0, type: 0},
...
//Repeating the above step for WireCube_2
for (var i =0; i < wireCubeLookups.length; i++)
{
   shapes.wireCube_2.points.push(cubeVerts[wireCubeLookups[i]]);
   shapes.wireCube_2.colors.push(violate);
}
...
window.onload = function init() {
  ...
  loadShape(shapes.wireCube_2, gl.LINE_STRIP);
  ...
  // I changed aspect parameter value as well as canvas width and height
  // and wrote the answer in the word document. 
  // Afterwards I changed them back to the way they were to continue with the next lab excercise steps.
  p = perspective(45.0, 1, 0.1, 100.0); 
//   p = ortho(-4, 4, -4, 4, 0.1, 20);
...
function render() {
   ...
   mv = mult(mv, translate(1,0,0)); // Translate the wired cube one unit in the positive x direction. 
   gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mv)));
   gl.drawArrays(shapes.wireCube.type, shapes.wireCube.start, shapes.wireCube.size);

   mv = mult(mult(mv, rotateY(45)), translate(0,1,0)); // Rotate the modelview matrix by 45 degrees around the Y axis.
   gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mv)));
   gl.drawArrays(shapes.wireCube_2.type, shapes.wireCube_2.start, shapes.wireCube_2.size);
